...
const ContentCreation = () => {
  ...
  const [selectedContenPoster, setSelectedContentPoster] = useState(null)
  ...
  const handleContentPosterChange = event => {
    setContentPoster(event.target.files[0])
    const file = event.target.files[0] // Get the first selected file
    if (file) {
      const imageUrl = URL.createObjectURL(file) // Create a URL for the selected image
      setSelectedContentPoster(imageUrl) // Set the state with the URL
    }
  }
  ...
  const postData = () => {
    ...
    formData.append('content_poster', contentPoster)
    ...
    if (contentName && contentName !== '') {
      fetch(constants.url + 'ott/add_content', {
        method: 'POST',
        headers: {
                ...
                >
                  ...
                  <Col md='6' className=''>
                      ...
                      <Input
                        ...
                        type='file'
                        accept='image/*' // Allow only image files
                        onChange={handleContentPosterChange}
                      ></Input>
                      {/* </FormGroup> */}
                    </Col>
                    {selectedContenPoster && (
                      <>
                        <Col md='6'>
                          <div>
                            <img
                              src={selectedContenPoster}
                              alt='Selected'
                              style={{
                                maxHeight: '300px',
                                maxWidth: '300px',
